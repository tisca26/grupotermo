...
class Number_to_letter_converter
{
    ...
    private function convertNumber($number, $miMoneda = null, $type)
    {
        ...
        $numberStrFill = str_pad($numberStr, 15, '0', STR_PAD_LEFT);
        $billones = substr($numberStrFill, 0, 3);
        $miles_millones = substr($numberStrFill, 3, 3);
        $millones = substr($numberStrFill, 6, 3);
        $miles = substr($numberStrFill, 9, 3);
        $cientos = substr($numberStrFill, 12);

        if (intval($billones) > 0) {
            if ($billones == '001') {
                $converted .= 'UN BILLON ';
            } else if (intval($billones) > 0) {
                $converted .= sprintf('%sBILLONES ', $this->convertGroup($billones));
            }
        }

        if (intval($miles_millones) > 0) {
            if ($miles_millones == '001') {
                $converted .= 'MIL ';
            } else if (intval($miles_millones) > 0) {
                $converted .= sprintf('%sMIL ', $this->convertGroup($miles_millones));
            }
        }
